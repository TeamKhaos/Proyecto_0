...
class ParallaxManager:
    def __init__(self, ancho, alto):
        ...
        self.alto = alto
        self.capas = []
        # Capa lejana (Morados/Azules para Nivel 2)
        self.capas.append([Estrella(random.randint(0, ancho), random.randint(0, alto), 
                                   random.uniform(0.1, 0.5), 1, NES_GRAY) for _ in range(100)])
        # Capa media
        self.capas.append([Estrella(random.randint(0, ancho), random.randint(0, alto), 
                                   random.uniform(0.6, 1.2), 2, NES_BLUE) for _ in range(50)])
        # Capa cercana
        self.capas.append([Estrella(random.randint(0, ancho), random.randint(0, alto), 
                                   random.uniform(1.5, 2.5), 3, NES_LIGHT_BLUE) for _ in range(20)])
...
class Nave:
    def __init__(self, x=None, y=None):
        ...
        self.velocidad = 7 # Ligeramente más rápida en nivel 2
        self.ancho = 64
        self.alto = 64
        self.vida = 100
        self.max_vida = 100
    ...
    def dibujar(self, pantalla):
        if self.frames[0]:
            ...
            if tiempo_actual - self.tiempo_creacion < 1500:
                ...
                frame_actual = 2 if moviendose else 1
...
class NivelDosScene:
    def __init__(self, nombre_jugador):
        ...
        self.enemy_manager = EnemyManager(nivel=2, target=self.nave)
        self.boss = Boss(target=self.nave)
        self.boss.vida = 200 # Boss más fuerte
        self.boss.max_vida = 200
        self.boss.shoot_delay = 40 # Ataca más rápido
        ...
        for event in eventos:
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    mouse_pos = event.pos
                    if self.pausa:
                        self.manejar_pausa_clic(mouse_pos)
                    elif self.game_over:
                        self.manejar_game_over_clic(mouse_pos)
                    elif self.victoria:
                        self.manejar_victoria_clic(mouse_pos)

    def manejar_pausa_clic(self, pos):
        jugar_rect = pygame.Rect(0, 0, 240, 60); jugar_rect.center = (self.centro_x, self.centro_y - 20)
        volver_rect = pygame.Rect(0, 0, 240, 60); volver_rect.center = (self.centro_x, self.centro_y + 60)
    ...
    def manejar_game_over_clic(self, pos):
        reintentar_rect = pygame.Rect(0, 0, 240, 60); reintentar_rect.center = (self.centro_x, self.centro_y)
        menu_rect = pygame.Rect(0, 0, 240, 60); menu_rect.center = (self.centro_x, self.centro_y + 80)
        from engine.scene_manager import SceneManager
        if reintentar_rect.collidepoint(pos):
            AudioManager.play_boton(); SceneManager.cambiar_escena(NivelDosScene(self.nombre))
    ...
    def manejar_victoria_clic(self, pos):
        btn_menu = pygame.Rect(0, 0, 240, 60); btn_menu.center = (self.centro_x, self.centro_y + 80)
    ...
    def disparar(self):
        self.balas_jugador.append(Bala(self.nave.x + self.nave.ancho // 2, self.nave.y, direccion=-1, color=NES_YELLOW))
    ...
    def actualizar(self):
        ...
        for b in self.balas_jugador:
            ...
            for e in self.enemy_manager.enemigos:
                if b.obtener_rect().colliderect(e.obtener_rect()):
                    ...
                    AudioManager.play_explosion(); self.enemy_manager.enemigos.remove(e)
        ...
        for b in self.balas_enemigas:
            if b.obtener_rect().colliderect(nave_rect):
                self.nave.recibir_dano(8) # Más daño en nivel 2
    ...
    def dibujar_ui(self, pantalla):
        ...
        if self.boss.aparecido:
            pantalla.blit(self.font_pquena.render("SÚPER JEFE NIVEL 2", True, NES_RED), (self.centro_x - 100, 20))
    ...
    def mostrar_menu_pausa(self, pantalla):
        ...
        pausa_texto = self.font.render("PAUSA - NIVEL 2", True, NES_RED)
    ...
    def mostrar_victoria(self, pantalla):
        ...
        t = self.titulo_font.render("¡NIVEL 2 COMPLETADO!", True, NES_GREEN)
        pantalla.blit(t, t.get_rect(center=(self.centro_x, self.centro_y - 100)))
        m = self.font_pquena.render("Has demostrado ser un gran piloto.", True, NES_WHITE)
